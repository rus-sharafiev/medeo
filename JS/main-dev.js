...
$('nav').load('nav.php', function() {
    $('#ЛАПАРОСКОПИЯ').parent().addClass('nav-active');
...
if ($('#mobile-menu').css('display') == 'none') {
    $(this).addClass('active');
    $('main').html("<div class='lds-ellipsis'><div></div><div></div><div></div><div></div></div>");
    var dir_data = $(this).parent().parent().parent().children().first().text();
    var data = $(this).text();
    $('main').load( 'search.php', { cat: data, dir: dir_data } );
    history.pushState({ cat: data, dir: dir_data }, "", "/" + data + "_(" + dir_data + ")");
} else {
...
$('nav > div > div:last-child > div > div:last-child > div').click( function() {
    ...
    var dir_data = $(this).parent().parent().parent().parent().children().first().text();
    $('main').load( 'search.php', { subcat: data1, cat: data2, dir: dir_data } );
    history.pushState({ subcat: data1, cat: data2, dir: dir_data }, "", "/" + data2 + "_" + data1 + "_(" + dir_data + ")");
