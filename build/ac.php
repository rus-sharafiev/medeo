...
// Category
    include 'login.php';
    $query = "SELECT direction, category FROM catalog WHERE category LIKE '$q%'";
    $cat_array = array();
    ...
    if ($cat_array != NULL) {
        ...
        foreach ($cat_array as list($dir, $category)) { ?>    
            <div class='ac-cat'>
                <div><?php printf ($category); ?></div>
                <div><?php printf ($dir); ?></div>
            </div><?php
        }
        ?> </div> <?php
    }
    ...
    <script>
        $('.ac-cat').click( function() {
            var cat_data = $(this).children().first().text();
            var dir_data = $(this).children().last().text();
            $('main').load( 'search.php', { cat: cat_data, dir: dir_data });
            $('.active').removeClass('active');
            history.pushState({ cat: cat_data, dir: dir_data }, "", "/" + cat_data + "_(" + dir_data + ")");
        });
    </script>
